	...
	protected void construct(Interfacing interfacing, SymbolTable symbolTable, Namespace namespace,
		...
		for(int m = 0; m < methods.length; m ++) {
			...
			if(method.result() == null)
				...
				if(typeResult.primitive == null && !method.result().variant()
					...
					sourcePrinter.println(typeResult.name(symbolResult, namespace) + " result;");
				else sourcePrinter.println(typeResult.name(symbolResult, namespace) + " /** Align here. **/");
				super.ioMethod(symbolTable, namespace, readSerializer, "input", 
						interfacing.host()? "*this" : "*host", 
						sourcePrinter, new Type[] { method.result() }, new String[] { "result" });
				sourcePrinter.println("return result;");
			}
